...
/*
 * This is an important class that enqueues requests and creates broadcast queue threads to respond them concurrently. If the local data is not available, it is necessary to forward the request to the local node's children. 11/29/2014, Bing Li
 * 
 * It works in the way like a scheduler or a dispatcher. That is why it is named. 11/29/2014, Bing Li
 */

// Created: 11/29/2014, Bing Li
public class BroadcastRequestDispatcher<Request extends BroadcastRequest, Response extends BroadcastResponse, RequestThread extends BroadcastRequestQueue<Request, Response>, RequestThreadCreator extends BroadcastRequestThreadCreatable<Request, Response, RequestThread>> extends Thread implements CheckIdleable
{
	 ...
	 */
	public BroadcastRequestDispatcher(FreeClientPool pool, String serverAddress, int serverPort, int poolSize, long keepAliveTime, RequestThreadCreator actionThreadCreator, int maxTaskSize, int maxThreadSize, long dispatcherKeepAliveTime)
	{
		this.pool = pool;
	 ...
	 */
	public BroadcastRequestDispatcher(FreeClientPool pool, String serverAddress, int serverPort, ThreadPool threadPool, RequestThreadCreator actionThreadCreator, int maxTaskSize, int maxThreadSize, long dispatcherKeepAliveTime)
	{
		this.pool = pool;
		this.serverAddress = new IPPort(serverAddress, serverPort);
		this.threadMap = new ConcurrentHashMap<String, RequestThread>();
