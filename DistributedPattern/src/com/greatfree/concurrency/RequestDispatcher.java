...
public class RequestDispatcher<Request extends ServerMessage, Stream extends OutMessageStream<Request>, Response extends ServerMessage, RequestThread extends RequestQueue<Request, Stream, Response>, ThreadCreator extends RequestThreadCreatable<Request, Stream, Response, RequestThread>> extends Thread implements CheckIdleable
{
	...
	{
		// Set the shutdown flag to be true. Thus, the loop in the dispatcher thread to schedule requests load is terminated. 11/04/2014, Bing Li
		this.workCollaborator.setShutdown();
		// Notify the dispatcher thread that is waiting for the requests to terminate the waiting. 11/04/2014, Bing Li 
								...
								try
								{
									...
									if (this.threadMap.get(selectedThreadKey).isFull())
									{
										...
										if (this.threadMap.size() < this.maxThreadSize)
										{
											...
											RequestThread thread = this.threadCreator.createRequestThreadInstance(this.maxTaskSize);
											// Save the newly created thread into the map. 11/04/2014, Bing Li
											this.threadMap.put(thread.getKey(), thread);
											// Enqueue the request into the queue of the newly created thread. Then, the request will be processed and responded by the thread. 11/04/2014, Bing Li
										...
										{
											// Force to put the request into the queue when the count of threads reaches the upper limit and each of the thread's queue is full. 11/04/2014, Bing Li
