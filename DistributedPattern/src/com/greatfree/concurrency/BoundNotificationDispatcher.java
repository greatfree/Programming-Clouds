...
public class BoundNotificationDispatcher<Notification extends ServerMessage, Binder extends MessageBindable<Notification>, NotificationThread extends BoundNotificationQueue<Notification, Binder>, ThreadCreator extends BoundNotificationThreadCreatable<Notification, Binder, NotificationThread>> extends Thread implements CheckIdleable
{
	...
	// The collaborator is used to pause the dispatcher when no notifications are available and notify to continue when new notifications are received. 11/26/2014, Bing Li
	...
	// A flag that indicates whether a local thread pool is used or a global one is used. Since it might get problems if the global pool is shutdown inside the class. Thus, it is required to set the flag. 11/26/2014, Bing Li
	 ...
	 */
	public BoundNotificationDispatcher(int poolSize, long keepAliveTime, Binder binder, ThreadCreator threadCreator, int maxTaskSize, int maxThreadSize, long dispatcherKeepAliveTime)
	{
	 ...
	 */
	public BoundNotificationDispatcher(ThreadPool threadPool, Binder binder, ThreadCreator threadCreator, int maxTaskSize, int maxThreadSize, long dispatcherKeepAliveTime)
	{
		this.key = Tools.generateUniqueKey();
		this.threadMap = new ConcurrentHashMap<String, NotificationThread>();
		this.notificationQueue = new LinkedBlockingQueue<Notification>();
	...
	public synchronized void enqueue(Notification notification)
	{
		...
		// Notify the dispatcher thread, which is possibly blocked when no requests are available, to keep working. 11/27/2014, Bing Li
								...
								try
								{
									...
									if (this.threadMap.get(selectedThreadKey).isFull())
									{
										...
										if (this.threadMap.size() < this.maxThreadSize)
										{
											...
											NotificationThread thread = this.threadCreator.createNotificationThreadInstance(this.maxTaskSize, this.key, this.binder);
											// Save the newly created thread into the map. 11/27/2014, Bing Li
											this.threadMap.put(thread.getKey(), thread);
											// Enqueue the notification into the queue of the newly created thread. Then, the notification will be processed by the thread. 11/27/2014, Bing Li
