 ...
 * 
 * Different from classic dispatchers, the interactive pool splits the threads into two types, the fast one and the low one. However, the splitting is performed upon interactions between the dispatcher and those threads. 11/21/2014, Bing Li
 * 
 */

// Created: 11/20/2014, Bing Li
public class InteractiveDispatcher<Task, Notifier extends Interactable<Task>, Function extends InteractiveQueue<Task, Notifier>, ThreadCreator extends InteractiveThreadCreatable<Task, Notifier, Function>, ThreadDisposer extends ThreadDisposable<Function>> implements CheckIdleable
{
	...
	public InteractiveDispatcher(int fastPoolSize, int slowPoolSize, int threadQueueSize, long idleTime, Notifier notifier, ThreadCreator creator, ThreadDisposer disposer)
	{
		...
		this.fastThreaderMap = new ConcurrentHashMap<String, Threader<Function, ThreadDisposer>>();
		this.slowThreaderMap = new ConcurrentHashMap<String, Threader<Function, ThreadDisposer>>();
	...
	public void enqueue(Task task)
	{
		...
		while (true)
		{
			...
			if (this.fastThreaderMap.size() < this.fastPoolSize)
			{
				...
				Threader<Function, ThreadDisposer> threader = new Threader<Function, ThreadDisposer>(thread, this.disposer);
	...
	public void checkIdle()
	{
		...
		for (Map.Entry<String, Threader<Function, ThreadDisposer>> threadEntry : this.fastThreaderMap.entrySet())
		{
