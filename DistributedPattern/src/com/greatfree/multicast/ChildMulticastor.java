...
public class ChildMulticastor<Message extends ServerMulticastMessage, MessageCreator extends ChildMulticastMessageCreatable<Message>> extends HashFreeObject
{
				...
				for (String childrenKey : tree.get(UtilConfig.LOCAL_KEY))
				{
					...
					if (allChildrenKeys != UtilConfig.NO_CHILDREN_KEYS)
					{
						...
						while (allChildrenKeys.size() > 0)
						{
							try
							{
								...
								this.clientPool.send(new IPPort(message.getChildrenNodeIP(childrenKey), this.clusterNodePort), forwardMessage);
					...
					else
					{
						...
						try
						{
							...
							this.clientPool.send(new IPPort(message.getChildrenNodeIP(childrenKey), this.clusterNodePort), message);
						}
						catch (IOException e)
						{
							/*
			...
			else
			{
				...
				for (Map.Entry<String, String> serverAddressEntry : message.getChildrenNodes().entrySet())
				{
					try
					{
						...
						this.clientPool.send(new IPPort(message.getChildrenNodeIP(serverAddressEntry.getValue()), this.clusterNodePort), message);
					}
					catch (IOException e)
					{
						/*
