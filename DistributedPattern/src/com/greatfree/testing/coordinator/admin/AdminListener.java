	...
	public void run()
	{
		...
		while (!super.isShutdown())
		{
			try
			{
				...
				// If the upper limit of IOs is not reached, a crawling server IO is initialized. A common Collaborator and the socket are the initial parameters. The shared common collaborator guarantees all of the crawling server IOs from a certain crawler could notify with each other with the same lock. Then, the upper limit of crawling server IOs is under the control. 11/27/2014, Bing Li
