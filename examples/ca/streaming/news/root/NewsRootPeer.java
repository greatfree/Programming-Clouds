...
class NewsRootPeer
{
	private Peer<NewsRootDispatcher> peer;
	
	private NewsRootPeer()
	{
	}
	
	private static NewsRootPeer instance = new NewsRootPeer();
	
	public static NewsRootPeer UNICAST()
	{
		if (instance == null)
		{
			instance = new NewsRootPeer();
			return instance;
		}
		else
		{
	...
	public void stop(long timeout) throws ClassNotFoundException, IOException, InterruptedException, RemoteReadException
	{
		...
		NewsRootMulticastor.STREAM().stop();
	...
	{
		this.peer = new Peer.PeerBuilder<NewsRootDispatcher>()
				...
				.dispatcher(new NewsRootDispatcher(RegistryConfig.DISPATCHER_THREAD_POOL_SIZE, RegistryConfig.DISPATCHER_THREAD_POOL_KEEP_ALIVE_TIME, RegistryConfig.SCHEDULER_THREAD_POOL_SIZE, RegistryConfig.SCHEDULER_THREAD_POOL_KEEP_ALIVE_TIME))
		...
		if (ipResponse.getIPs() != null)
		{
			...
			NewsRootMulticastor.STREAM().start(this.peer.getClientPool(), MulticastConfig.ROOT_BRANCH_COUNT, MulticastConfig.SUB_BRANCH_COUNT, MulticastConfig.BROADCAST_REQUEST_WAIT_TIME, this.peer.getPool());
			NewsRootMulticastor.STREAM().broadcastNotify(new RootIPAddressBroadcastNotification(new IPAddress(this.peer.getPeerID(), this.peer.getPeerIP(), this.peer.getPort())));
		}
		else
		{
			NewsRootMulticastor.STREAM().start(this.peer.getClientPool(), MulticastConfig.ROOT_BRANCH_COUNT, MulticastConfig.SUB_BRANCH_COUNT, MulticastConfig.BROADCAST_REQUEST_WAIT_TIME, this.peer.getPool());
