...
class SearchPeer
{
	private Peer<SearchDispatcher> peer;

	private SearchPeer()
	{
	}
	
	private static SearchPeer instance = new SearchPeer();
	
	public static SearchPeer ROOT()
	{
		if (instance == null)
		{
			instance = new SearchPeer();
			return instance;
		}
		else
		{
	...
	{
		this.peer = new Peer.PeerBuilder<SearchDispatcher>()
				...
				.dispatcher(new SearchDispatcher(RegistryConfig.DISPATCHER_THREAD_POOL_SIZE, RegistryConfig.DISPATCHER_THREAD_POOL_KEEP_ALIVE_TIME, RegistryConfig.SCHEDULER_THREAD_POOL_SIZE, RegistryConfig.SCHEDULER_THREAD_POOL_KEEP_ALIVE_TIME))
		...
		if (ipResponse.getIPs() != null)
		{
			...
			SearchMulticastor.ROOT().start(this.peer.getClientPool(), MulticastConfig.ROOT_BRANCH_COUNT, MulticastConfig.SUB_BRANCH_COUNT, MulticastConfig.BROADCAST_REQUEST_WAIT_TIME, this.peer.getPool());
			SearchMulticastor.ROOT().broadcastNotify(new RootIPAddressBroadcastNotification(new IPAddress(this.peer.getPeerID(), this.peer.getPeerIP(), this.peer.getPort())));
		}
		else
		{
