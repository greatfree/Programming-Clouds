package org.greatfree.dsf.container.cs.multinode.client;
...
import org.greatfree.dsf.container.cs.multinode.message.AddPartnerNotification;
import org.greatfree.dsf.container.cs.multinode.message.ChatPartnerRequest;
import org.greatfree.dsf.container.cs.multinode.message.ChatRegistryRequest;
import org.greatfree.dsf.container.cs.twonode.client.ChatMaintainer;
	...
	public void send(int option) throws ClassNotFoundException, RemoteReadException, IOException, InterruptedException
	{
		...
		switch (option)
		{
			case MenuOptions.REGISTER_CHATTING:
				chatRegistryResponse = (ChatRegistryResponse)StandaloneClient.CS().read(ChatConfig.CHAT_SERVER_ADDRESS, ChatConfig.CHAT_SERVER_PORT, new ChatRegistryRequest(ChatMaintainer.CS_CONTAINER().getLocalUserKey(), ChatMaintainer.CS_CONTAINER().getLocalUsername(), ChatMaintainer.CS_CONTAINER().getLocalUsername() + " is a great & free guy!"));
				System.out.println("Chatting registry status: " + chatRegistryResponse.isSucceeded());
				break;
				
			case MenuOptions.SEARCH_USER:
				chatPartnerResponse = (ChatPartnerResponse)StandaloneClient.CS().read(ChatConfig.CHAT_SERVER_ADDRESS, ChatConfig.CHAT_SERVER_PORT, new ChatPartnerRequest(ChatMaintainer.CS_CONTAINER().getPartnerKey()));
