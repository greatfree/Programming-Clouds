package org.greatfree.dip.cluster.cs.twonode.client;
...
import org.greatfree.dip.cluster.original.cs.twonode.message.ChatNotification;
	...
	public void sent(int option)
	{
		switch (option)
		{
			case ChatOptions.TYPE_CHAT:
				...
				ChatClient.CONTAINER().asyncNotify(new ChatNotification(ChatTools.getChatSessionKey(ChatMaintainer.CS().getLocalUserKey(), ChatMaintainer.CS().getPartnerKey()), message, ChatMaintainer.CS().getLocalUserKey(), ChatMaintainer.CS().getLocalUsername(), ChatMaintainer.CS().getPartnerKey(), ChatMaintainer.CS().getPartner()));
				break;
