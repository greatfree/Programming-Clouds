...
final class AsyncReaderPoolInteractTester
{
	...
	public static void main(String[] args) throws InterruptedException
	{
		...
		Query dstQuery = new Query("hello destination query!");

		IntermediateTask it = new IntermediateTask(ra, srcQuery, readerB, dstQuery);
//		IntermediateTask it = new IntermediateTask(ra, readerB, srcQuery.getCollaboratorKey());
		Runner<IntermediateTask> dstRunner = new Runner<IntermediateTask>(it);
		dstRunner.start();
		
		WaitTask wt = new WaitTask(rb, dstQuery.getCollaboratorKey());
//		WaitTask wt = new WaitTask(rb, srcQuery.getCollaboratorKey());
