...
public class CoordinatorMulticastReader
{
	...
	public Set<String> searchKeyword(String keyword)
	{
		try
		{
			...
			SearchKeywordBroadcastReader reader = this.searchKeywordBroadcastReaderPool.get(new SearchKeywordBroadcastReaderSource(MemoryServerClientPool.COORDINATE().getPool(), ServerConfig.ROOT_MULTICAST_BRANCH_COUNT, ServerConfig.MULTICAST_BRANCH_COUNT, new SearchKeywordBroadcastRequestCreator(), ServerConfig.DISTRIBUTE_DATA_WAIT_TIME));
			// Get the collaborator key of the reader. The key is unique to the reader. 11/29/2014, Bing Li
