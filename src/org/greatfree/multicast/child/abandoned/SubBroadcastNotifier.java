...
/*
 * This is the multicasting code to run on a child node. That is why it is called the SubMulticastor. 11/11/2014, Bing Li
 */

// Created: 05/13/2017, Bing Li
abstract class SubBroadcastNotifier<Message extends OldMulticastMessage, MessageCreator extends ChildBroadcastNotificationCreatable<Message>> extends HashFreeObject
{
...
//	private int clusterNodePort;
	 ...
	 */
//	public SubMulticastor(FreeClientPool clientPool, int treeBranchCount, int clusterServerPort, MessageCreator messageCreator)
	public SubBroadcastNotifier(String localIPKey, FreeClientPool clientPool, int treeBranchCount, MessageCreator messageCreator)
	{
...
//		if (message.getChildrenIPs() != UtilConfig.NO_IPS)
		...
		{
			/*
			for (IPAddress ip : message.getChildrenIPs().values())
			{
...
//										this.clientPool.send(new IPPort(message.getIP(childrenKey)), forwardMessage);
										IPAddress ip = childrenFromRoot.get(childrenKey);
//										this.clientPool.send(new IPResource(ip.getPeerKey(), ip.getPeerName(), ip.getIP(), ip.getPort()), forwardMessage);
										this.clientPool.send(new IPResource(ip.getIP(), ip.getPort()), forwardMessage);
...
//								this.clientPool.send(new IPPort(message.getIP(childrenKey)), message);
								IPAddress ip = childrenFromRoot.get(childrenKey);
//								this.clientPool.send(new IPResource(ip.getPeerKey(), ip.getPeerName(), ip.getIP(), ip.getPort()), message);
								this.clientPool.send(new IPResource(ip.getIP(), ip.getPort()), message);
							}
							catch (IOException e)
							{
								/*
...
//							this.clientPool.send(new IPResource(entry.getValue().getPeerKey(), entry.getValue().getPeerName(), entry.getValue().getIP(), entry.getValue().getPort()), message);
							this.clientPool.send(new IPResource(entry.getValue().getIP(), entry.getValue().getPort()), message);
						}
						catch (IOException e)
						{
							/*
...
//		else
//		{
//			System.out.println("No children IPs ...");
//		}
	}
}
