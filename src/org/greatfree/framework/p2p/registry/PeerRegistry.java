...
//	private final static Logger log = Logger.getLogger("org.greatfree.framework.p2p.registry");
	...
	public IPAddress getAddress(String peerKey)
	{
		...
		{
			return new IPAddress(peerKey, account.getPeerName(), account.getIP(), account.getPeerPort());
		}
	...
	public Map<String, IPAddress> getIPPorts()
	{
		...
		for (PeerAccount entry : this.accounts.values())
		{
			...
			{
				ips.put(entry.getPeerKey(), new IPAddress(entry.getPeerKey(), entry.getPeerName(), entry.getIP(), entry.getPeerPort()));
			}
		}
		return ips;
	}
	...
	public Map<String, IPAddress> getIPPorts(List<String> peerKeys)
	{
		...
		for (String entry : peerKeys)
		{
			...
			{
				ips.put(entry, new IPAddress(entry, this.accounts.get(entry).getPeerName(), this.accounts.get(entry).getIP(), this.accounts.get(entry).getPeerPort()));
			}
		}
		return ips;
	}
	...
	public Map<String, IPAddress> getIPPorts(Set<String> peerKeys)
	{
		...
		{
			ips.put(key, new IPAddress(key, this.accounts.get(key).getPeerName(), this.accounts.get(key).getIP(), this.accounts.get(key).getPeerPort()));
//			log.info("ip = " + ips.get(key));
		}
