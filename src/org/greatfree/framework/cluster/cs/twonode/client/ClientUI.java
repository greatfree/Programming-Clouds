	...
	public void send(int option) throws ClassNotFoundException, RemoteReadException, IOException
	{
		...
		switch (option)
		{
			case MenuOptions.REGISTER_CHATTING:
				response = (CollectedClusterResponse)ChatClient.CONTAINER().read(new ChatRegistryRequest(ChatMaintainer.CS().getLocalUserKey(), ChatMaintainer.CS().getLocalUsername(), ChatMaintainer.CS().getLocalUsername() + " is a great & free guy!"));
