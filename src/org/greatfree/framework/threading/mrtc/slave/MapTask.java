...
{
	private final static Logger log = Logger.getLogger("org.greatfree.framework.threading.mrtc.master");
	...
	{
		log.info("=> MAP_TASK: local ID = " + NodeIDs.ID().getLocalName());
...
//		log.info("MAP_TASK: " + mn.getPath());
		...
		*/
		...
		log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: path:");
		log.info("============================================");
		...
		if (MRStates.CONCURRENCY().isCDFulfilled(mn.getMRSessionKey(), mn.getCD()))
		{
			...
			log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: CD = " + mn.getCD() + " is fulfilled ...");
			...
			{
				log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: MAP continued ...");
				...
				log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: slaveKeys size = " + slaveKeys.size());
//				String rp = NodeIDs.ID().getRandomSlaveKey();
				...
				log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: RP = " + NodeIDs.ID().getSlaveName(rp));
				try
				{
//					String rpThreadKey = Slave.THREADING().obtainThread(rp);
					...
					log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: RP ThreadKey = " + rpThreadKey);
					...
					for (String entry : slaveKeys)
					{
						...
						log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: Slave = " + NodeIDs.ID().getSlaveName(entry) + " got " + threadKeys.size() + " threads for mapping ...");
					...
					for (String entry : slaveKeys)
					{
						...
						log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: MAP to Slave: " + NodeIDs.ID().getSlaveName(entry) + " with " + threadKeys.size() + " threads");
...
//						Slave.THREADING().assignTask(entry, new ReduceNotification(mn.getMRKey(), threadKeys, MRStates.CONCURRENCY().getPath(mn.getMRKey()), ThreadConfig.TIMEOUT, rp, rpThreadKey, mn.getCurrentHop(), mn.getMaxHop(), cd, !rp.equals(NodeIDs.ID().getLocalKey())));
			...
			{
				log.info(NodeIDs.ID().getLocalName() + " => MAP_TASK: DONE to Master: " + Slave.THREADING().getMasterName());
				try
				{
